...
| v2.16 | 2026-08-12 | Backend | **PLANNED: production_batch_layout join table — satu batch untuk N cutting layouts (multi-fabric).** Masalah saat ini: ProductionBatch tied ke satu CuttingLayout (satu roll). Untuk pouch 3 kain, user harus buat 3 batch terpisah — stok naik 3× dan HPP hanya mencatat satu kain. **Perubahan yang diperlukan:** (1) **Schema:** tabel baru `production_batch_layout (id, production_batch_id FK CASCADE, cutting_layout_id FK, sort_order)` — satu batch bisa link ke N layouts; `production_batch.cutting_layout_id` (flat field) di-drop setelah migrasi data ke join table. (2) **POST /production-batches:** request berubah dari `{ cutting_layout_id }` ke `{ cutting_layout_ids: [UUID] }`; backward-compat validator menerima singular form dan konversi ke array. Batch item creation: group `CuttingLayoutItem` by `product_size_id` across all layouts → `qty_suggested = MIN(per-layout qty)` (bottleneck), `fabric_cost_per_piece = SUM(cost_per_piece)` (total fabric HPP). (3) **POST /production-batches/{id}/confirm:** deduct `remaining_length_cm` dari SEMUA linked purchases (bukan hanya satu); set `cutting_layout.status = "used"` untuk semua linked layouts. (4) **GET response:** `cutting_layout_ids: [UUID]` ditambahkan; `cutting_layout_id` menjadi computed alias ke elemen pertama (backward compat). **Yang tidak berubah:** batch item table structure, stock ledger logic (tetap satu entry per product_size), non-fabric HPP (labor/overhead/pooled/hardware), semua endpoint lain. Lihat `doc/implement-v2.16-multi-layout-batch.txt` untuk spec lengkap + test cases. |
