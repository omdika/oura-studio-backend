...
GET    /products/{sku}/sizes
  -- (updated v2.9: full field list — iOS uses this endpoint in fetchSizeToProductMap() for both
  --  Produk list enrichment AND batch confirmation RINCIAN HPP. All fields below required.)
  response per item: {
  ...
  -- production_stock_qty / manual_stock_qty: open conflict (see v2.9 revision note).
  --   iOS declares these as Int? and treats null as 0. Stock-source breakdown hidden if absent.
