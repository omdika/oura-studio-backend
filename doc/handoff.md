...
| v2.15 | 2026-08-12 | Backend | **PLANNED: pattern_spec_fabric join table — N fabrics per PatternSpec (belum diimplementasikan).** Migrasi dari model "satu PatternSpec per kain" ke "satu PatternSpec dengan N fabric layers." Root cause: field `fabric_material_id`, `cut_width_cm`, `cut_height_cm`, `rotation_allowed` saat ini flat di tabel `pattern_spec` — membatasi satu kain per spec. **Schema migration:** (1) Buat tabel `pattern_spec_fabric (id UUID PK, pattern_spec_id UUID FK pattern_spec.id ON DELETE CASCADE, material_id UUID FK material.id, cut_width_cm FLOAT NOT NULL, cut_height_cm FLOAT NOT NULL, rotation_allowed BOOL DEFAULT TRUE, fabric_label TEXT, sort_order INT DEFAULT 0)`; (2) Migrasi data: untuk setiap row `pattern_spec` existing, insert 1 row ke `pattern_spec_fabric` dengan nilai field lama; (3) Drop kolom `fabric_material_id, cut_width_cm, cut_height_cm, rotation_allowed` dari `pattern_spec`. **Gabungkan mode dihapus:** saat ini N PatternSpec berbagi `productSizeId` (gabungkan workaround); setelah migrasi, setiap `productSizeId` hanya punya satu PatternSpec aktif (bisa dengan N `pattern_spec_fabric` rows). Data existing gabungkan mode perlu dikonsolidasi: ambil semua PatternSpec dengan `productSizeId` yang sama dan `is_active=true`, merge fabric rows-nya ke PatternSpec pertama, hapus sisanya. **API changes:** (1) `POST /pattern-specs` — body berubah dari flat `{fabric_material_id, cut_width_cm, cut_height_cm}` ke `{fabrics: [{material_id, cut_width_cm, cut_height_cm, rotation_allowed, fabric_label?}]}` (array, min 1 item); (2) `GET /pattern-specs` + `GET /pattern-specs/{id}` — response `fabrics` sudah array di spec (frontend model sudah siap), hanya perlu diisi dari join tabel; (3) `PUT/PATCH /pattern-specs/{id}` — terima array `fabrics`, full-replace fabric rows (delete existing, insert new — karena `used_in_batch_count` ada di PatternSpec level bukan fabric level). **Optimizer changes:** `POST /cutting-optimizer/suggest` candidates sekarang punya `fabrics[]` — saat optimizer dipanggil untuk satu roll (material_id X), filter candidates yang punya `fabric_material_id=X` di antara fabric layers-nya; response harus tetap per-roll; frontend akan panggil suggest N kali (sekali per kain) dan tampilkan bottleneck. `POST /cutting-optimizer/layouts` tidak berubah. **Batch changes:** `POST /production-batches` (dari layout) mencakup satu roll kain; frontend akan POST N kali untuk N kain → N batch; konfirmasi batch tetap per-batch. **ATAU** tambah konsep `production_plan` (lihat Pendekatan 2 di frontend v3.5) untuk konfirmasi sekali — keputusan belum final. **HPP fix:** `_fabric_cost_per_unit()` sekarang SUM atas semua `pattern_spec_fabric` rows untuk spec tersebut — setiap kain berkontribusi ke HPP fabric total. **Tidak berubah:** produk 1 kain (1 fabric layer) — code path identik, hanya data di-store di join table. |
