...
GET    /products/{sku}/sizes
  -- response: array of ProductSize objects, each including fabric_variant_name (nullable)
  -- client computes display label: fabric_variant_name != null ? "{size_label} · {fabric_variant_name}" : size_label
  -- v2.8: each item also includes latest_hpp_breakdown + margin_pct (same shape as the by-ID
  --   endpoint below) -- needed so the Produk list/detail screens can drive Price Advisor
  --   without an extra round trip. null until this product_size has a confirmed production batch.
